##########################
# Test script to check if decisions can be possibly converted to targeted ones
...
def test_non_targeted_decisions(test_runner: object):
    ...
    for filename in glob.iglob(filepath + "**/*.txt", recursive=True):
        ...
        if "unlock_decision" in text_file:
            pattern_matches = re.findall(r".*unlock_decision.*", text_file)
    ...
    ResultsReporter.report_results(results=sorted(results), message="Recheck the piece of code and convert to targeted decision if possible.")
