...
def format_vp_endonym_file(username, mod_name):
    ...
    filename = f'{runner.full_path_to_mod}common\\scripted_localisation\\00 - Scripted State Endonyms.txt'
    ...
    for i in vp_loc_entries:
        if i.count("localization_key =") > 1:       # Not touching entries with only 1 loc key
            x = re.findall("text = \\{\\n\\t*localization_key = (STATE_.*)", i)[0]
            y = re.findall("name = GetStateName.*\\n", i)[0]
