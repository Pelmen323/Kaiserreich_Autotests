...
FILES_TO_SKIP = ('00_demobilization_decisions.txt', 
                ...
                'Cyrenaica_decisions.txt',      # Caravans
                'Germany_decisions.txt',        # Empty decisions with modifier
                'Hunan_decisions.txt',          # Until reworked
...
def test_check_decisions_ai_factors(filepath: str):
    ...
    for file in glob.glob("*.txt"):
        ...
        if expected_num_of_ai_factors > ai_will_do_counter:
            ...
            results_dict[file] = f'Huh? We found {ai_will_do_counter} ai factors and only {expected_num_of_ai_factors} selectable decisions and missions. Lets recheck that file... Probably not all decisions have icons or missions that are not selectable have ai factors'
