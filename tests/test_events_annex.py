...
def test_check_unsupported_title_desc_combination(test_runner: object):
    ...
    for event in events_code:
        ...
        for option in options:
            ...
            # 5 All other options that aren't give_to_overlord, integration, or occupation should have is_subject = no in the trigger
            # elif "occupation" not in name:
            #     try:
            #         trigger = re.findall(r'(^\t\ttrigger = \{.*?^\t\t\})', option, flags=re.DOTALL | re.MULTILINE)[0]
            #         if "is_subject = no" not in trigger:
            #             results.append(f'{event_id} {name} - missing is_subject = no in the trigger')
            #     except IndexError:
            #         results.append(f'{event_id} {name} - missing is_subject = no in the trigger - trigger block is missing')
        ...
        if event_options[-1] != "annex.occupation":
            ...
            try:
                ...
                if "annexations_is_allowed_to_occupy = yes" not in trigger:
                    results.append(f'{event_id} - last event option annex.occupation - should have only annexations_is_allowed_to_occupy = yes as the trigger')
            except IndexError:
                results.append(f'{event_id} - last event option annex.occupation - should have only annexations_is_allowed_to_occupy = yes as the trigger - trigger block is missing')
