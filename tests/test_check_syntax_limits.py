    ...
    for filename in glob.iglob(filepath + '**/*.txt', recursive=True):
        ...
        if len(every_country_count) > 0:
            for match in every_country_count:
                match = match[0]                                            # Counter empty capture groups
                if 'reverse_add_opinion_modifier' in match: continue
                if '#unit limits setup' in match: continue
                if 'former_reichspakt_member' in match: continue
                container[match] = os.path.basename(filename)
        ...
        if len(every_country_count) > 0:
            for match in every_other_country_count:
                match = match[0]                                            # Counter empty capture groups
                if 'kr_similar_ideology_close' in match: continue
                if 'ai_events.100' in match: continue
                if 'ai_events.101' in match: continue
                container[match] = os.path.basename(filename)
