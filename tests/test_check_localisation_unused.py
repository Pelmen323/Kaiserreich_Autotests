...
def test_check_unused_loc_keys(test_runner: object):
    ...
    for filename in glob.iglob(filepath + '**/*.yml', recursive=True):
        if "KR_Vanilla_Override_l_english" in filename:
            continue
        if "ideas_l_english" in filename:
            continue
        if "music" in filename:
            continue
        if "lar_operations" in filename:
            continue
        if "autonomy_l" in filename:
            continue
        if "decisions_l_english" in filename:
            continue
        if "events_l_english" in filename:
            continue
        if "factions_l_english" in filename:
            continue
        if "focus_l_english" in filename:
            continue
        if "technology_sharing" in filename:
            continue
        if "00_General_l_english" in filename:
            continue
        if "War_l_english" in filename:
            continue
        if "war_l_english" in filename:
            continue
        if "00_Map_Victory" in filename:
            continue
        if "00_Map_States" in filename:
            continue
        if "_Equip" in filename:
            continue
        if "_Adjacency" in filename:
            continue
        if "st_manager" in filename:
            continue
    ...
    if unused_loc_keys != {}:
        logging.warning(
            "There are unused loc keys, check 'loc_files_issues.txt' on desktop")
