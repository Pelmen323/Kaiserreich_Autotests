...
# Checks if generals/admirals use correct stats and correct sums of stats 1 lvl - 4, 2 lvl - 7, 3 lvl -10 and so on
...
def test_characters_unit_leader_stats(test_runner: object):
    ...
    for char in characters:
        ...
        if any([corps_commander_role, field_marshal_role]):
            try:
                atk_skill = re.findall(r"attack_skill = (\d+)", char)[0]
                dfs_skill = re.findall(r"defense_skill = (\d+)", char)[0]
                pln_skill = re.findall(r"planning_skill = (\d+)", char)[0]
                log_skill = re.findall(r"logistics_skill = (\d+)", char)[0]
            ...
            try:
                atk_skill = re.findall(r"attack_skill = (\d+)", char)[0]
                dfs_skill = re.findall(r"defense_skill = (\d+)", char)[0]
                mnvr_skill = re.findall(r"maneuvering_skill = (\d+)", char)[0]
                coord_skill = re.findall(r"coordination_skill = (\d+)", char)[0]
        ...
        try:
            skill_level = int(re.findall(r"skill = (\d+)", char)[0])
        except IndexError:
    ...
    ResultsReporter.report_results(results=results, message="Issues with characters stats sums encountered.")
