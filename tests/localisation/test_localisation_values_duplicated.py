...
FALSE_POSITIVES_STATE = ["Georgia", "Maine", "Morava", "Santa Cruz", "Córdoba"]
FALSE_POSITIVES_VP = [
    "Birmingham",
    "Charleston",
    "Concepcion",
...
list_of_filepaths = [
    ["00 Map States l_english.yml", FALSE_POSITIVES_STATE],
    ["00 Map Victory Points l_english.yml", FALSE_POSITIVES_VP]
]


@pytest.mark.parametrize("filepath", list_of_filepaths)
def test_find_duplicated_keys(test_runner: object, filepath):
    pathlib_path = str(Path(test_runner.full_path_to_mod) / "localisation" / "english" / "KR_common" / filepath[0])
    loc_dict = Localization.get_all_loc_keys(test_runner=test_runner, lowercase=False, return_keys_from_specific_file=pathlib_path)
