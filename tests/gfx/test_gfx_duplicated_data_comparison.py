##########################
# Test script to check duplicated GFXs by comparing image data
...
@pytest.mark.skip(reason="Resource-heavy test, for local execution only")
def test_gfx_duplicated(test_runner: object):
    filepath = str(Path(test_runner.full_path_to_mod) / "gfx") + "/"
    results = []
    images_list = []
